  ...
  try {
    ...
    switch (action) {
      ...
      case 'getAllVoluntarios':
        result = getAllVoluntarios();
        break;
      case 'addVoluntario':
        result = addVoluntario(data);
        break;
...
function sendMonthlyMeetingEmail(email, firstName, token) {
  ...
  var htmlContent = '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"></head>' +
    ...
    '<p style="margin:0 0 10px 0;color:#001A55;font-size:15px;"><strong>Fecha:</strong> El 15 de cada mes</p>' +
...
function getProfessionalByToken(token) {
  ...
  // Fallback: buscar en la hoja de Voluntarios
...
var VOLUNTARIOS_SHEET = 'Voluntarios';

/**
 * Busca un voluntario por token en la hoja Voluntarios.
 * Devuelve objeto compatible con getProfessionalByToken para reusar el sistema legal.
 */
function getVolunteerByToken(token) {
  ...
  for (var i = 1; i < data.length; i++) {
    if (String(data[i][0] || '').trim().toLowerCase() === tokenNorm) {
      return {
        ...
        email: data[i][2],
        telefono: data[i][3] || '',
        tipo: data[i][4] || 'voluntario',
        estado: data[i][5] || 'activo',
        fechaRegistro: data[i][6] || '',
        isVoluntario: true
      };
    }
  }
  ...
  try {
    ...
    if (vSheet && vSheet.getLastRow() > 1) {
      ...
      for (var i = 1; i < vData.length; i++) {
        if (vData[i][0]) {
          volunteers.push({
            token: String(vData[i][0]),
            nombre: String(vData[i][1] || ''),
            email: String(vData[i][2] || ''),
    ...
    // Enriquecer con estado del contrato (hoja Aceptaciones_Legales, docId=VOLUNTARIO)
    var aceptSheet = getSS().getSheetByName('Aceptaciones_Legales');
    var signed = {};
    if (aceptSheet && aceptSheet.getLastRow() > 1) {
      var aData = aceptSheet.getDataRange().getValues();
      for (var j = 1; j < aData.length; j++) {
        if (String(aData[j][1] || '').toUpperCase() === 'VOLUNTARIO') {
          var tk = String(aData[j][4] || '').trim().toLowerCase();
          signed[tk] = {
            folio: aData[j][0],
            version: String(aData[j][2]),
            fecha: aData[j][3] ? new Date(aData[j][3]).toISOString() : ''
          };
        }
      }
    }
    ...
    volunteers.forEach(function(v) {
      var key = String(v.token).trim().toLowerCase();
      v.contratoFirmado = !!signed[key];
      v.contratoInfo = signed[key] || null;
    });
...
/**
 * Agrega un nuevo voluntario a la hoja Voluntarios y genera su token único.
 */
function addVoluntario(data) {
  try {
    ...
    var tipo = String(data.tipo || 'voluntario').trim();

    if (!nombre || !email) return { success: false, message: 'Nombre y correo son obligatorios.' };

    var sheet = getSS().getSheetByName(VOLUNTARIOS_SHEET);
    if (!sheet) {
      sheet = getSS().insertSheet(VOLUNTARIOS_SHEET);
      sheet.appendRow(['Token', 'Nombre', 'Email', 'Telefono', 'Tipo', 'Estado', 'Fecha_Registro']);
    }

    if (sheet.getLastRow() > 1) {
      var existing = sheet.getDataRange().getValues();
      for (var i = 1; i < existing.length; i++) {
        if (String(existing[i][2] || '').trim().toLowerCase() === email) {
          return { success: false, message: 'Ya existe un voluntario con ese correo.' };
        }
      }
    }

    var token = 'VOL-' + Utilities.getUuid().replace(/-/g, '').substring(0, 12).toUpperCase();
    sheet.appendRow([token, nombre, email, telefono, tipo, 'activo', new Date()]);

    try { logAction(token, email, 'Voluntario registrado: ' + nombre, 'Voluntario_Registro', '', '', 'Admin'); } catch(e) {}

    return { success: true, token: token, nombre: nombre, email: email };
  } catch (e) {
    return { success: false, message: e.toString() };
  }
}
